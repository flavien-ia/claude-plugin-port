...
export function buildCodex({ srcDir, files, src, fsx, opts, log, warnings, generator, port = {} }) {
  ...
  fsx.write(path.join(installDir, ".claude-plugin-to-codex.json"), JSON.stringify({
    generator, target: "codex", source: srcDir, sourceVersion: src.version, date: new Date().toISOString(),
    renamedSkills: converted.renamedSkills, excludedSkills: converted.excludedSkills,
  }, null, 2) + "\n");
  log("- manifest .codex-plugin/plugin.json");
  log(`- ${converted.skillCount} skills, ${converted.files.length} files (scripts and templates included)`);
  ...
  if (converted.skillCount > 40 && !opts.has("--short-descriptions")) {
    log(`\nHint: ${converted.skillCount} skills. Codex caps the skill catalog at 2% of the context and shortens descriptions past that; --short-descriptions keeps the first sentence of each.`);
  }
